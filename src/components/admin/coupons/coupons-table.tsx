...
import { Eye, Pencil, Search, Percent, DollarSign, ChevronUp, ChevronDown } from "lucide-react";
...
export function CouponsTable({
  ...
  const [detailCoupon, setDetailCoupon] = useState<AdminCouponListItem | null>(null);
            ...
            {coupons.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-16 text-center text-sm text-base-content/40">
                  {search || status ? "No coupons match your filters" : "No coupons yet"}
                </td>
              </tr>
            ) : (
              coupons.map((coupon) => (
                <tr key={coupon.id} className="hover">
                  ...
                  <td>
                    <span className="flex items-center gap-1 text-sm">
                      {coupon.type === "percentage" ? (
                        <><Percent className="size-3 text-base-content/40" /> Percentage</>
                      ) : (
                        <><DollarSign className="size-3 text-base-content/40" /> Fixed</>
                      )}
                    </span>
                  </td>
                  ...
                  <td className="hidden text-sm md:table-cell">
                    ...
                    {coupon.maxUsage !== null && (
                      <span className="text-base-content/40"> / {coupon.maxUsage}</span>
                    )}
                  </td>
                  <td className="hidden text-sm text-base-content/50 md:table-cell">
                    {coupon.expiresAt ? formatDateShort(coupon.expiresAt) : <span className="text-base-content/30">&mdash;</span>}
                  </td>
                  <td>
                    <Badge
                  ...
                  <td>
                    <div className="flex items-center justify-end gap-1">
                      ...
                      >
                        {coupon.isActive ? (
                          <svg className="size-4 text-error" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                          </svg>
                        ) : (
                          <svg className="size-4 text-success" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      >
        ...
        <div className="space-y-4 px-6 py-4 text-sm">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Code</p>
              <p className="mt-0.5 font-mono font-medium">{coupon.code}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Type</p>
              <p className="mt-0.5 capitalize">{coupon.type}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Value</p>
              <p className="mt-0.5 font-medium">
                {coupon.type === "percentage" ? `${coupon.value}%` : formatCurrency(coupon.value)}
              </p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Status</p>
              <p className="mt-0.5">
                <Badge variant={coupon.isActive ? "success" : "error"} size="xs">
                  {coupon.isActive ? "Active" : "Inactive"}
                </Badge>
              </p>
            </div>
            {coupon.description && (
              <div className="col-span-2">
                <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Description</p>
                <p className="mt-0.5">{coupon.description}</p>
              </div>
            )}
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Usage</p>
              <p className="mt-0.5">
                {coupon.currentUsage ?? 0}
            ...
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Min Purchase</p>
              <p className="mt-0.5">{coupon.minPurchase ? formatCurrency(coupon.minPurchase) : <span className="text-base-content/30">&mdash;</span>}</p>
            </div>
            {coupon.maxDiscount && (
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Max Discount</p>
                <p className="mt-0.5">{formatCurrency(coupon.maxDiscount)}</p>
              </div>
            )}
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Start Date</p>
              <p className="mt-0.5">{coupon.startDate ? formatDateShort(coupon.startDate) : <span className="text-base-content/30">&mdash;</span>}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Expires</p>
              <p className="mt-0.5">{coupon.expiresAt ? formatDateShort(coupon.expiresAt) : <span className="text-base-content/30">&mdash;</span>}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Created</p>
              <p className="mt-0.5">{formatDateShort(coupon.createdAt)}</p>
            </div>
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-base-content/50">Updated</p>
